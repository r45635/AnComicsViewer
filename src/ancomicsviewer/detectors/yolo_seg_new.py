...
class YoloSegPanelDetector(BasePanelDetector):
    def __init__(self, weights: str, conf: float = 0.15, iou: float = 0.6, imgsz_infer: int = 1280, rtl: bool=False, row_band_frac: float = 0.06):
    ...
    def detect_panels(self, qimage: QImage, page_point_size: QSizeF) -> List[QRectF]:
        ...
        def overlaps_title(rect: QRectF) -> bool:
            """Safer title filtering - only drop if IoU>0.6 and title area is <25% of panel."""
            a = (rect.left()*s, rect.top()*s, rect.right()*s, rect.bottom()*s)
            for t in titles:
                if iou_xyxy(a, t) > 0.60:
                    # keep if panel is much larger than the title block
                    tx1,ty1,tx2,ty2 = t
                    ta = (tx2-tx1)*(ty2-ty1)
                    pa = (a[2]-a[0])*(a[3]-a[1])
                    if ta / max(pa,1) < 0.25:
        ...
        return sort_reading_order(keep, page_point_size, self.reading_rtl, self.row_band_frac)
