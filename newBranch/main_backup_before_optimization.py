...
import sys, os, json, yaml, hashlib, pickle
...
class DetectionCache:
    """Cache for detection results to avoid recomputation when revisiting pages"""
    
    def __init__(self, cache_dir: str = ".detection_cache"):
        self.cache_dir = Path(cache_dir)
        self.cache_dir.mkdir(exist_ok=True)
        self._memory_cache: Dict[str, Tuple[List, List]] = {}  # In-memory cache for speed
    
    def _get_cache_key(self, pdf_path: str, page_index: int, page_width: int, 
                      page_height: int, model_name: str, imgsz_max: int, tile_tgt: int) -> str:
        """Generate cache key from page and detection parameters"""
        cache_str = f"{pdf_path}:{page_index}:{page_width}x{page_height}:{model_name}:{imgsz_max}:{tile_tgt}"
        return hashlib.sha1(cache_str.encode()).hexdigest()
    
    def get(self, pdf_path: str, page_index: int, page_width: int, page_height: int,
            model_name: str, imgsz_max: int, tile_tgt: int) -> Optional[Tuple[List, List]]:
        """Get cached detection results (panels, balloons)"""
        key = self._get_cache_key(pdf_path, page_index, page_width, page_height, 
                                 model_name, imgsz_max, tile_tgt)
        ...
        if cache_file.exists():
            try:
                with open(cache_file, 'rb') as f:
                    ...
                    self._memory_cache[key] = result  # Cache in memory too
                    return result
            except Exception:
    ...
    def set(self, pdf_path: str, page_index: int, page_width: int, page_height: int,
            model_name: str, imgsz_max: int, tile_tgt: int, 
            panels: List, balloons: List) -> None:
        """Store detection results in cache"""
        key = self._get_cache_key(pdf_path, page_index, page_width, page_height,
                                 model_name, imgsz_max, tile_tgt)
        
        result = (panels, balloons)
...
class PdfYoloViewer(QMainWindow):
    ...
    def _run_detection(self):
        """
        Fast and stable inference with caching.
        - Uses detection cache to avoid recomputation
        - Single pass when possible, limited tiling when needed
        ...
        config = {
            'panel_conf': 0.18, 'balloon_conf': 0.22, 'balloon_area_min_pct': 0.0006,
            'balloon_min_w': 30, 'balloon_min_h': 22, 'tile_target': 896,
            'tile_overlap': 0.20, 'iou_merge': 0.55, 'panel_containment_merge': 0.65,
            'max_balloons': 12, 'page_margin_inset_pct': 0.015, 'imgsz_max': 1280,
            'max_det': 400, 'force_tiling': False
        }
        try:
        ...
        # --- Check cache first ---
        model_name = os.path.basename(getattr(self.model, 'ckpt_path', 'unknown'))
        cache_result = self.detection_cache.get(
            pdf_path=getattr(self.pdf, 'name', 'unknown') if self.pdf else 'unknown',
        ...
        if cache_result is not None:
            panels, balloons = cache_result
            self.dets = [Detection(c, r, p) for c, p, r in panels + balloons]
            num_panels = len([d for d in self.dets if d.cls == 0])
            num_balloons = len([d for d in self.dets if d.cls == 1])
            self.status.showMessage(f"Page {self.page_index + 1}: panels={num_panels}, balloons={num_balloons} "
                                  f"(imgsz={IMGSZ_MAX}, cache=hit)")
            return

        # --- Fast inference strategy ---
        # Ensure imgsz = min(IMGSZ_MAX, max(W, H)) with no dynamic half-sizes
        max_side = max(H, W)
        imgsz = min(IMGSZ_MAX, max_side)
        imgsz -= imgsz % 32  # Ensure divisible by 32
        ...
        if max_side <= TILE_TGT * 1.15 and not FORCE_TILING:
            ...
            # Limited tiling - cap at 3x3 maximum
            if max_side <= TILE_TGT * 2:
                nx, ny = 2, 2
            elif max_side <= TILE_TGT * 3:
                nx, ny = 3, 3
            else:
                # For very large images, still cap at 3x3
                nx, ny = 3, 3
            
            tiles_info = f"tiles={nx}x{ny}"
            ...
            y = 0
            while y < H and len(all_dets) < MAX_DET:  # Stop if we have enough detections
                x = 0
                yh = min(H, y + int(H / ny + TILE_OVERLAP * H / ny))
                while x < W and len(all_dets) < MAX_DET:
        ...
        # 1. Clamp and filter by confidence and area
        filtered_dets = []
        for c, p, r in all_dets:
            r_clamped = r.intersected(inset_page_rect)
            if r_clamped.isEmpty():
                continue
                
            area_pct = (r_clamped.width() * r_clamped.height()) / PAGE_AREA
            
            if c == 0:  # Panel
                if p >= PANEL_CONF and area_pct >= 0.012:  # 1.2% min area
                    filtered_dets.append((c, p, r_clamped))
            elif c == 1:  # Balloon
                if (p >= BAL_CONF and area_pct >= BAL_MIN_PCT and 
                    r_clamped.width() >= BAL_MIN_W and r_clamped.height() >= BAL_MIN_H):
                    filtered_dets.append((c, p, r_clamped))

        # 2. IoU Merge
        merged = []
        for c, p, r in sorted(filtered_dets, key=lambda x: x[1], reverse=True):
            is_merged = False
        ...
        # Limit balloons
        if len(balloons) > MAX_BAL:
            balloons = sorted(balloons, key=lambda x: x[1], reverse=True)[:MAX_BAL]
        ...
        num_balloons = len([d for d in self.dets if d.cls == 1])
        self.status.showMessage(f"Page {self.page_index + 1}: panels={num_panels}, balloons={num_balloons} "
                              f"(imgsz={imgsz}, {tiles_info}, cache=miss)")

    def _apply_fusion_algorithms(self, panels, balloons, config, W, H):
        """Apply fusion algorithms if enabled in config"""
        # This function is now integrated into _run_detection for simplicity
        # All fusion logic has been moved to the main detection function
